...
#         HF_REPO = "Sammm3/mentalwell-emotion-model"  # ← set this once

#         # Try local first; fall back to HuggingFace Hub
#         if os.path.exists(model_path):
#             print(f"✅ Loading emotion model from local path: {model_path}")
#             self.tokenizer = AutoTokenizer.from_pretrained(model_path, local_files_only=True)
#             self.model = AutoModelForSequenceClassification.from_pretrained(model_path, local_files_only=True)
#         else:
#             print(f"⬇️  Local model not found. Downloading from HuggingFace: {HF_REPO}")
...
#             # Save locally so next run is fast
...
class EmotionDetector:
    ...
    def __init__(
        ...
        if os.path.exists(model_path):
            ...
            print(
                f"⬇️ Local model not found. Downloading from HuggingFace: {HF_REPO}"
            )
        ...
        if os.path.exists(mapping_file):
            ...
            print("✅ Loaded emotion_mapping.json")

        else:
            print(
                "⚠️ emotion_mapping.json not found, using model config labels"
            )

            self.id2emotion = self.model.config.id2label
            self.emotion2id = self.model.config.label2id

        print(f"✅ Emotion Detector loaded on {device}")
        print(f"   Emotions: {list(self.id2emotion.values())}")
